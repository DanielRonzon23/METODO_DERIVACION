...
double Derivacion::funcion0(double x){//original function
    return (-(0.1*pow(x,4))-(0.15*pow(x,3))-(0.5*pow(x,2))-(0.25*x)+(1.2));
}
double  Derivacion::funciond1(double x){//first derivate
    return (-(0.4*pow(x,3))-(0.45*pow(x,2))-(x)-(0.25));
}
double  Derivacion::funciond2(double x){//second derivate
    return (-(1.2*pow(x,2))-(0.9*x)-(1));
}
double  Derivacion::funciond3(double x){//third derivate
    return (-(2.4*x)-(0.9));
}
double  Derivacion::funciond4(double x){//fourth derivate
    return (-2.4);
}
void Derivacion::procesar_d(){//starts the process after clicking buttom Ejecutar
...
void Derivacion::borrar_d(){//cleans all
